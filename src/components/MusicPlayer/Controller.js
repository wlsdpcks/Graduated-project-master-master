...
export default function Controller({onNext, onPrv}) {
  ...
  const returnPlayBtn = () => {
    switch (isPlaying.current) {
      case 'playing':
        return <Icon color="orange" name="pause" size={45} />;
      case 'paused':
        return <Icon color="orange" name="play-arrow" size={45} />;
      default:
        return <ActivityIndicator size={45} color="orange" />;
    }
  };
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity onPress={onPrv}>
        <Icon color="orange" name="skip-previous" size={45} />
      </TouchableOpacity>
      <TouchableOpacity onPress={onPlayPause}>
        {returnPlayBtn()}
      </TouchableOpacity>
      <TouchableOpacity onPress={onNext}>
        <Icon color="orange" name="skip-next" size={45} />
      </TouchableOpacity>
    </View>
  );
}
